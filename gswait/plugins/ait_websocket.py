...
class Session(object):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        """
        assert self._numConnections > 0  # nosec
        self._numConnections -= 1
...
class SessionStore(dict):
    ...
    def add_telemetry(self, uid, packet):
        ...
        pkt_name = pkt_defn.name if pkt_defn is not None else pkt_defn
        decoded = tlm.Packet(pkt_defn, data=bytearray(packet))
        item = (uid, decoded.toJSON())
        ...
        for session in self.values():
            ...
            item = (uid, decoded.toJSON(), session.tlm_counters[pkt_name])
    ...
    def create(self):
        ...
        log.info(f"created session id: {session.id}")
...
class AITWebsocket(Plugin):
    global playback

    def __init__(self, inputs, outputs, zmq_args=None, **kwargs):
        super(AITWebsocket, self).__init__(inputs, outputs, zmq_args, **kwargs)
    ...
    def process(self, input_data, topic=None):
        ...
        if not processed:
            if topic is not None and "telem_stream" in topic:
                self.process_telem_msg(input_data)
                processed = True
    ...
    def init(self):
        ...
        port = int(getattr(self, "port", 8001))
